...
export RESET_PROXY_POOL_CMD="pkill -x proxy || true; sleep 1; NEON_INTERNAL_CA_FILE=/tmp/compute-ca.crt RUST_LOG=proxy LOGFMT=text cargo run -p proxy --bin proxy --features testing -- --auth-backend cplane-v1 --auth-endpoint http://127.0.0.1:3010 --tcp-pool-enabled true --tcp-pool-mode transaction --tcp-pool-max-conns-per-key 100 --tcp-pool-idle-timeout 30s --redis-auth-type plain --redis-plain 'redis://127.0.0.1:6379' -c server.crt -k server.key --endpoint-rps-limit 5000@1s --endpoint-rps-limit 5000@60s --endpoint-rps-limit 5000@600s --wake-compute-limit 5000@1s --wake-compute-limit 5000@60s --wake-compute-limit 5000@600s >/tmp/neon-proxy-bench.log 2>&1 &"
